...
- Workspace- and folder-scoped values of `ai-features.*` preferences are ignored altogether. This covers tool confirmation overrides (`ai-features.chat.toolConfirmation`), language model aliases (`ai-features.modelSettings.languageModelAliases`), agent settings (`ai-features.agentSettings`), the directories AI tools may read outside the workspace (`ai-features.workspaceFunctions.allowedExternalPaths`), and the allow- and deny-lists of the [shell execution tool](/docs/user_ai/#command-allowlist-and-denylist). The effective values fall back to your user or default settings, so an untrusted workspace cannot lower these guardrails. While the trust state is still being determined, the restricted values are used.
